...
class API_Client_FTX:
    ...
    def export_to_csv(self):
        df = self.df
        return df.to_csv(r'C:\Users\lubor\Documents\coding stuff\data\MOVE\\'+str(self.symbol)+r'.csv')
...
def hist_tick_MOVE_data():
    ...
    for d in dates_list:
        ...
        for h in hours:
            ...
            individual_file.to_csv(r'C:\Users\lubor\Documents\coding stuff\data\MOVE\\'+str(symbol)+r'.csv')
            print(str(h+1) + ' hours of data from ' + '' + symbol + ' downloaded.')
            time.sleep(1)
        print('Data of MOVE-' + d + ' downloaded.')
    MOVE_tick_daily_data.to_csv(r'C:\Users\lubor\Documents\coding stuff\data\MOVE\\MOVE_tick_data.csv')
